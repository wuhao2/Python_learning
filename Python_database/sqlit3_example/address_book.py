import os
import sqlite3
from sqlite3 import connect #用于与sqlit3建立连接

class MySqliteDb(object):#定义一个连接数据库的类
    """Sqlite3 Db Class"""
    ...
    def __enter__(self):#协议方法，建立连接
        self.getCursor()
        return self.curs

    def __exit__(self, exc_type, exc_val, exc_tb):#协议方法， 关闭连接
        if exc_val:
...
class Contact:
    def __init__(self,name='',home_tel='',office_tel='',mobile_phone='',memo=''):
        ...
        self.__change()#用于标记当前类的属性是否发生了变化
    #保存联系人
    def save(self):
        if self.id == -1:
