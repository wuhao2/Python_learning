...
class DoublyLinkedList(object):
    ...
    def append(self, data):
        ...
        :return:
        """
        new_node = Node(data)  # 实例化一个新的节点
        pre = self.tail.pre  # 定义一个指针pre

        pre.next = new_node   # 将指针挂接 四步走
        new_node.pre = pre
    ...
    def insert(self, index, data):
        ...
        next_node = self.get(index)  # 得到
        if next_node:
            '''双向链表插入节点6步'''
            insert_node = Node(data)  # 实例化一个要插入的节点
...
if __name__ == '__main__':
    ls = DoublyLinkedList()  # 实例化一个链表，存在一个头结点与尾节点
    print(len(ls))
